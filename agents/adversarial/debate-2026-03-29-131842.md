...
- **Date**: 2026-03-29-131842
- **Target**: claude
- **Arbiter**: none
- **Topic**: "Review this architect subagent prompt for a spec-driven workflow. Give the top 5 concrete improvements to make it more reliable, self-contained, and cross-CLI portable. Focus on false-pass, ambiguity, missing input handling, traceability, and progress-file semantics. Keep it concise and actionable.
...
### 1. Coverage gaps don't block — false-pass by design

Step 7 says "call out gaps" but doesn't prevent writing a complete-looking `design.md`. A task planner downstream will trust it as done.

**Fix:** Add a `status: complete | incomplete` field to the frontmatter. If any AC/NFR is unmapped in the coverage matrix, status MUST be `incomplete` and the agent must list the unmapped IDs in a `## Unresolved Gaps` section. Downstream agents check `status` before proceeding.

### 2. `requirements_sha: "not-captured"` defeats drift detection

The escape hatch makes the SHA optional in practice — an LLM will take it every time it can't trivially compute the hash. The whole point of the field (detecting requirements drift between design and implementation) is lost.

**Fix:** Make it mandatory. Provide the exact method: `sha256sum basePath/requirements.md | cut -d' ' -f1`. If the agent can't compute it (no shell access), require it to hash the file contents inline or use a deterministic placeholder like the first+last 20 chars of the file. Remove the `"not-captured"` option entirely.

### 3. No structured failure signal from Step 0

"Stop and report the blocking issue explicitly" — report WHERE? If the agent writes nothing, downstream can't distinguish "hasn't run yet" from "ran and failed." This is the classic silent-failure antipattern.

**Fix:** On validation failure, write a minimal `design.md` with:
```yaml
---
status: blocked
...
---
```
And update `.progress.md` with `phase: design, status: blocked`. This gives any downstream agent (in any CLI) a machine-readable signal.

### 4. `.progress.md` format is undefined — guaranteed drift

"Append a learning to the Learnings section" assumes a structure that no agent defines. Each phase agent will create/append with a different format, making `.progress.md` an unreliable mess after 2-3 phases.

**Fix:** Define the canonical schema once (ideally in a shared spec, but at minimum in this prompt):
```markdown
## Phase Log
| Phase | Status | Agent | Timestamp |
|-------|--------|-------|-----------|
| design | complete | architect | 2026-03-29T... |

## Learnings
- **[design]** <learning text>
```
And specify: append a row to Phase Log, append a bullet prefixed with `[design]` to Learnings. Don't create freeform sections.

### 5. Input validation is subjective — LLMs always pass themselves

"Contains explicit AC and FR/NFR identifiers you can trace" and "no obvious contradiction" are judgment calls. An LLM will almost always conclude inputs are "good enough" rather than halt.

**Fix:** Replace with concrete, mechanical checks:
- `requirements.md` contains at least one `AC-\d+\.\d+` pattern
- `requirements.md` contains at least one `FR-\d+` or `NFR-\d+` pattern
- `idea.md` has a `spec` or `name` field in frontmatter
- All three files are non-empty (>10 lines)

If any check fails, trigger the structured failure signal from point 3. Mechanical checks remove the LLM's discretion to hand-wave past broken inputs.

---

**Honorable mention:** The example Mermaid diagram (`A→B→C`) biases the LLM toward linear architectures. Replace it with a more realistic multi-path example or remove it entirely and just say "include a Mermaid component diagram."
