...
- Codex subprocess routing now uses concrete Dell-verified model IDs (`gpt-5.4-mini`, `gpt-5.4`, `gpt-5.5`, `gpt-5.6-sol`) under `workspace-write`.
- Executors are pure implementers: `agents/executor.md` and `agents/executor-subprocess.md` modify task files and verify only; the coordinator owns git commits and tracking updates.
...
- Real probes confirm `codex exec -m <model> -s workspace-write -- ...` works for all Codex tiers and `claude -p --model claude-opus-4-8 --effort high -- "..."` runs successfully.
- End-to-end canary tasks passed through Codex subprocess, Claude frontier subprocess, and the agent contract path; each produced parseable `TASK_COMPLETE`, then coordinator-owned post-processing re-ran Verify and committed with the exact task message.
